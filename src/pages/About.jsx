...
const About = () => {
  ...
  return (
    <div className="pt-20 bg-bg-primary">
        ...
        <div className="container-custom relative z-10 px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 md:gap-6 max-w-6xl mx-auto px-4">
            {statsData.map((stat, idx) => (
              <div key={idx} className="bg-bg-secondary/80 border-2 border-primary p-3 md:p-6 text-center hover:scale-105 transition-transform">
        ...
        <div className="container-custom px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-16 items-stretch">
            <div className="relative">
              <div className="aspect-square bg-gradient-to-br from-primary to-secondary rounded-2xl md:rounded-3xl p-6 md:p-8 relative overflow-hidden min-h-[300px]">
                ...
                <div className="relative z-10 h-full flex flex-col justify-end">
                  <h2 className="text-4xl md:text-7xl font-heading font-black text-txt-primary uppercase leading-tight mb-3 md:mb-4">
                    {missionVisionData.mission.title.split(' ')[0]}<br />{missionVisionData.mission.title.split(' ')[1]}
                  </h2>
                  <p className="text-base md:text-2xl text-txt-primary/90 leading-relaxed">
            ...
            <div className="space-y-6 md:space-y-8">
              <div className="bg-bg-secondary backdrop-blur-sm border-l-4 border-primary p-6 md:p-8">
                <h3 className="text-2xl md:text-4xl font-heading font-black text-txt-primary mb-3 md:mb-4 uppercase">{missionVisionData.vision.title}</h3>
                <p className="text-sm md:text-xl text-txt-secondary leading-relaxed">
                  {missionVisionData.vision.description}
                </p>
              </div>

              <div className="bg-bg-secondary backdrop-blur-sm border-l-4 border-primary p-6 md:p-8">
                <h3 className="text-2xl md:text-4xl font-heading font-black text-txt-primary mb-3 md:mb-4 uppercase">{missionVisionData.commitment.title}</h3>
                <p className="text-sm md:text-xl text-txt-secondary leading-relaxed">
      ...
      {/* Timeline - Journey */}
      <section className="timeline-section section-padding bg-bg-primary">
        <div className="container-custom px-4">
          ...
          <div className="max-w-5xl mx-auto space-y-4 md:space-y-6">
            {journeyData.map((item, idx) => (
              <div key={idx} className="timeline-item relative group">
                <div className="bg-bg-secondary backdrop-blur-sm border-2 border-gray-300 hover:border-primary rounded-xl md:rounded-2xl p-4 md:p-6 transition-all relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-start gap-4 md:gap-6">
                    <div className="w-16 h-16 md:w-20 md:h-20 bg-primary flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                      <span className="text-xl md:text-2xl font-heading font-black text-txt-primary">{item.year}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg md:text-xl lg:text-2xl font-heading font-black text-txt-primary mb-2 md:mb-3 break-words leading-tight">{item.title}</h3>
                      <p className="text-xs md:text-sm text-txt-secondary leading-relaxed break-words">{item.description}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="container-custom px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto">
            {valuesData.map((value, idx) => (
              <div
                key={idx}
                className="bg-bg-secondary backdrop-blur-sm border-2 border-gray-200 hover:border-primary rounded-xl md:rounded-2xl p-4 md:p-6 lg:p-8 transition-all group relative overflow-hidden min-h-[200px] md:min-h-[240px]"
              >
                ...
                <div className="relative z-10 h-full flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="text-lg md:text-xl lg:text-2xl font-heading font-black text-txt-primary mb-2 md:mb-3 break-words leading-tight">{value.title}</h3>
                    <p className="text-xs md:text-sm text-txt-secondary leading-relaxed break-words">{value.description}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Head Coach Section */}
      <section className="team-section section-padding bg-bg-primary">
        <div className="container-custom px-4">
          <h2 className="text-2xl md:text-4xl lg:text-6xl font-heading font-black text-txt-primary mb-4 md:mb-8 text-center break-words">
            {teamSectionData.heading.split(' ').slice(0, 1).join(' ')} <span className="text-primary">{teamSectionData.heading.split(' ').slice(1).join(' ')}</span>
          </h2>
          <p className="text-base md:text-xl lg:text-2xl text-txt-secondary text-center max-w-3xl mx-auto mb-8 md:mb-16">
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-6 md:gap-12 mb-8 md:mb-16">
              ...
              <div className="team-card relative group order-2 md:order-1">
                <div className="aspect-[3/4] md:aspect-[4/5] relative overflow-hidden border-4 border-gray-300 group-hover:border-primary transition-all">
                  <img
                    src={headCoach.img}
                    alt={headCoach.name}
              ...
              <div className="order-1 md:order-2 space-y-4 md:space-y-6">
                <div>
                  <h3 className="text-3xl md:text-4xl lg:text-6xl font-heading font-black text-txt-primary mb-2 break-words">{headCoach.name}</h3>
                  <div className="text-base md:text-xl text-primary font-bold uppercase">{headCoach.role}</div>
                  <p className="text-sm md:text-lg text-txt-muted italic mt-2">{headCoach.tagline}</p>
                </div>

                {/* Stats */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                  {headCoach.stats.map((stat, idx) => (
                    <div key={idx} className="bg-bg-secondary border border-primary p-2 md:p-4 text-center">
                      <div className="text-lg md:text-2xl lg:text-3xl font-heading font-black text-primary">{stat.value}</div>
                      <div className="text-xs md:text-sm text-txt-muted break-words">{stat.label}</div>
                    </div>
                  ))}
                </div>

                {/* Bio */}
                <div className="bg-bg-secondary backdrop-blur-sm border border-gray-300 p-4 md:p-6">
                  <p className="text-sm md:text-base text-txt-secondary leading-relaxed">{headCoach.bio}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 md:gap-8 mb-8 md:mb-16">
              {/* Certifications */}
              <div className="bg-bg-secondary backdrop-blur-sm border-2 border-gray-300 p-4 md:p-6 lg:p-8">
                <h4 className="text-xl md:text-2xl lg:text-3xl font-heading font-black text-txt-primary mb-4 md:mb-6">CERTIFICATIONS</h4>
                <div className="space-y-3 md:space-y-4">
                  {headCoach.certifications.map((cert, idx) => (
                    <div key={idx} className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm md:text-base font-bold text-txt-primary break-words">{cert.name}</div>
                        <div className="text-xs md:text-sm text-txt-muted break-words">{cert.org}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Specializations */}
              <div className="bg-bg-secondary backdrop-blur-sm border-2 border-gray-300 p-4 md:p-6 lg:p-8">
                <h4 className="text-xl md:text-2xl lg:text-3xl font-heading font-black text-txt-primary mb-4 md:mb-6">SPECIALIZATIONS</h4>
                <div className="space-y-3">
                  {headCoach.specializations.map((spec, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <div className="w-2 h-2 bg-primary flex-shrink-0"></div>
                      <span className="text-sm md:text-base text-txt-secondary break-words">{spec}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Achievements */}
            <div className="bg-bg-secondary border-2 border-gray-300 p-4 md:p-6 lg:p-8 mb-8 md:mb-16">
              <h4 className="text-xl md:text-2xl lg:text-3xl font-heading font-black text-txt-primary mb-4 md:mb-6 text-center">KEY ACHIEVEMENTS</h4>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                {headCoach.achievements.map((achievement, idx) => (
                  <div key={idx} className="bg-bg-secondary border border-gray-300 hover:border-primary p-3 md:p-4 transition-all flex items-center gap-3">
                    ...
                    <span className="text-xs md:text-sm text-txt-secondary break-words">{achievement}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-primary/20 to-transparent border-2 border-primary p-6 md:p-8 lg:p-12">
              <h4 className="text-xl md:text-2xl lg:text-3xl font-heading font-black text-txt-primary mb-4 md:mb-6">TRAINING PHILOSOPHY</h4>
              <p className="text-sm md:text-base lg:text-lg text-txt-secondary leading-relaxed italic">&ldquo;{headCoach.philosophy}&rdquo;</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding bg-bg-secondary">
        <div className="container-custom">
          <h2 className="text-6xl md:text-8xl font-heading font-black text-txt-primary mb-16 uppercase text-center">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {facilityData.map((item, idx) => (
              <div key={idx} className="relative aspect-video overflow-hidden border-4 border-gray-300 hover:border-primary transition-all group">
                <img
                  src={item.image}
                  alt={item.title}
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-bg-primary/80 via-bg-primary/30 to-transparent"></div>
                <div className="absolute bottom-8 left-8 right-8">
                  <h3 className="text-4xl font-heading font-black text-txt-primary mb-2 uppercase">{item.title}</h3>
                  <p className="text-xl text-txt-secondary">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="section-padding bg-bg-primary relative overflow-hidden">
        ...
        <div className="container-custom relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-6xl md:text-9xl font-heading font-black text-txt-primary mb-8 uppercase leading-none">
