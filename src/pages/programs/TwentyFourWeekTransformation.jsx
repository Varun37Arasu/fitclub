...
import { twentyFourWeekData } from '../../data/programs';
import MoneyBackBadge from '../../components/MoneyBackBadge';

gsap.registerPlugin(ScrollTrigger);

const TwentyFourWeekTransformation = () => {
  useEffect(() => {
        ...
        <div className="container-custom relative z-10 px-4 flex-1 flex items-center justify-center">
          <div className="program-hero-title text-center max-w-full w-full">
            ...
            <h1 className="text-4xl md:text-6xl lg:text-8xl font-heading font-black leading-[0.95] tracking-tighter mb-6 md:mb-8 break-words px-2 max-w-full">
              ...
              <span className="block text-[#ff4500] neon-text">{twentyFourWeekData.hero.title.split(' ').slice(1).join(' ')}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center w-full sm:w-auto px-4">
              <Link to="/payment?plan=24-week" className="btn-primary text-sm md:text-lg lg:text-xl px-8 md:px-12 py-3 md:py-6 text-center w-full sm:w-auto">
                START NOW → {twentyFourWeekData.hero.price}
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="section-padding bg-[#1a1a1a]">
        <div className="container-custom px-4">
          ...
          <div className="max-w-3xl mx-auto space-y-3 md:space-y-6">
            {twentyFourWeekData.trainingPhases.map((phase, idx) => (
        ...
        <div className="container-custom px-4">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-heading font-black text-white mb-4 md:mb-6 leading-tight break-words px-2">
              {twentyFourWeekData.cta.heading.split(' ').slice(0, 2).join(' ')} <span className="text-[#ff4500]">{twentyFourWeekData.cta.heading.split(' ').slice(2).join(' ')}</span>
            </h2>
            ...
            <Link to={twentyFourWeekData.cta.buttonLink} className="btn-primary text-sm md:text-lg px-8 md:px-12 py-4 md:py-6 inline-block">
              {twentyFourWeekData.cta.buttonText}
            </Link>
          </div>
        </div>
      </section>
...
export default TwentyFourWeekTransformation;
